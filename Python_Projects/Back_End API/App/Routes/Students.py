...
from App.Schemas.Student import StudentCreate, StudentUpdate, StudentSelfUpdate, StudentResponse, StudentListResponse
...
def update_my_profile(
    ...
    # Convert StudentSelfUpdate → StudentUpdate so the service layer is unified
    admin_update = StudentUpdate(**update_data.model_dump(exclude_unset=True))
    return Student_service.update_student(db, student["id"], admin_update, current_user.email)
...
def get_all(
    ...
    """
    Admin: returns all students with filters & pagination.
    Student: returns only their own profile wrapped in the list format.
    """
    if current_user.role == "student":
        student = Student_service.get_student_by_user_id(db, current_user.id)
...
def get_one(
    ...
    """Get a single student — students can only see themselves"""
...
def update(
    ...
    - Student: can only update their own record, and only phone, email, password.
    """
    if current_user.role == "student":
        student = Student_service.get_student_by_id(db, student_id)
        if student["user_id"] != current_user.id:
            raise HTTPException(status_code=403, detail="Access denied")
        # Strip fields students are not allowed to change
        allowed = update_data.model_dump(exclude_unset=True)
        forbidden = {k for k in allowed if k not in ("phone", "email", "password")}
        if forbidden:
            raise HTTPException(
                status_code=403,
                detail=f"Students cannot update: {', '.join(forbidden)}"
            )

    return Student_service.update_student(db, student_id, update_data, current_user.email)
...
def delete(
    ...
    return Student_service.delete_student(db, student_id, performed_by=current_user.email)
