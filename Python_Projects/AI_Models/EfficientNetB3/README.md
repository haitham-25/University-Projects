# 🌿 Plant Disease Classification Model (EfficientNetB3)

A deep learning model for classifying plant diseases using Transfer
Learning with EfficientNetB3. This model identifies 38 different plant
disease conditions across multiple crops.

------------------------------------------------------------------------

## 🎯 Overview

This project implements a plant disease classification system using a
pretrained EfficientNetB3 model for high accuracy and efficiency.

### Crops Covered:

Apple, Blueberry, Cherry, Corn, Grape, Orange, Peach, Pepper, Potato,
Raspberry, Soybean, Squash, Strawberry, Tomato

### Conditions:

Includes multiple diseases (bacterial spot, blight, rust, mildew, virus
infections) and healthy classes.

------------------------------------------------------------------------

## 📊 Dataset

-   Total Images: 54,284
-   Number of Classes: 38
-   Image Size: 300×300 (recommended for EfficientNetB3)
-   Training: 34,741
-   Validation: 8,686
-   Test: 10,857

------------------------------------------------------------------------

## 🏗️ Model Architecture

Base Model: EfficientNetB3 (ImageNet Pretrained)

Input (300x300x3) ↓ EfficientNetB3 (Frozen Initially) ↓
GlobalAveragePooling2D ↓ BatchNormalization ↓ Dropout (0.5) ↓ Dense
(512, ReLU) ↓ Dropout (0.3) ↓ Dense (38, Softmax)

------------------------------------------------------------------------

## ⚙️ Requirements

Python 3.8+ tensorflow\>=2.10.0 keras\>=2.10.0 numpy pandas matplotlib
seaborn scikit-learn Pillow

------------------------------------------------------------------------

## 🚀 Installation

1.  Clone repository git clone `<your-repo-url>`{=html}

2.  Install dependencies pip install -r requirements.txt

3.  Organize dataset plant_dataset/ ├── Apple\_**Apple_scab/ ├──
    Apple**\_Black_rot/ └── ...

------------------------------------------------------------------------

## 📖 Training

-   Freeze EfficientNetB3 base model
-   Train for 25--30 epochs
-   Optimizer: Adam
-   Loss: Categorical Crossentropy
-   Learning Rate: 0.0001
-   Use EfficientNet preprocessing function

------------------------------------------------------------------------

## 📈 Model Performance

Validation Accuracy: \~97--98% Batch Size: 32 Epochs: 25--30

------------------------------------------------------------------------

## 📱 Deployment

Model converted to TensorFlow Lite.

Input: Shape: (1, 300, 300, 3) Type: float32 Preprocessing: efficientnet
preprocess_input()

Output: Shape: (1, 38) Softmax probability distribution

------------------------------------------------------------------------

## ⚠️ Limitations

-   Larger size than MobileNet
-   Higher GPU memory usage
-   Slower training time

------------------------------------------------------------------------

## 🔮 Future Improvements

-   Fine-tuning top layers
-   Quantization (INT8 / Float16)
-   Grad-CAM visualization
-   Advanced augmentation (MixUp, CutMix)
-   Model pruning

------------------------------------------------------------------------

## 📄 License

MIT License

------------------------------------------------------------------------

Last Updated: February 2026 Model Version: 1.0 Framework: TensorFlow 2.x
